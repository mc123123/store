...
def query():
    ...
    savequery = bank_query(username, password, money, country, province, street, door)
...
def bank_query(username, password, money, country, province, street, door):
    a = users.values()  # 字典里所有值给了a
    if username in users:  # 判断字典里面用户中有没有用户名
        for i in a:  # 遍历所有值
...
def bank_withdraw_money(username,password,money):
    for value in users.values():
        # 判断是否已经存在
        if username not in users.keys(): # 这种方式只判断是否在字典的键里存在
            return 1
        # 密码不对
...
def withdraw_money():
    ...
    if status1==1:
        ...
        print("取款成功！您当前的余额为：",users[username]["money"])
...
def savemoney():
    ...
    if returnvalue == 1:
        print("存钱成功！！！您存的金额为:", money, "￥", "余额为:", users[username]["money"], "￥")
        print(users)
    else:
        print("查无此用户！！！")
